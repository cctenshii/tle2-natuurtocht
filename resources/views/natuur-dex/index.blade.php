                        ...
                        <div x-show="openAccordion === {{ $category->id }}" x-collapse class="pt-4">
                            @forelse($category->grouped_items as $subGroup => $items)
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    @foreach($items as $item)
                                        <a href="{{ route('cards.show', $item->id) }}" class="block">
                                            <div
                                                class="bg-yellow-100 border border-yellow-200 rounded-lg p-2 text-center shadow hover:shadow-lg hover:scale-105 transition">
                            ...
                            @if($loop->first)
                                <div class="flex justify-center items-center gap-4 mt-8">
                                    <img src="https://placehold.co/100x40/DDD/777?text=Natuurmonumenten"
                                         alt="Natuurmonumenten" class="h-8">
